...
export function setupMessageSending(displayLatestMessages) {
  document.getElementById("sendMessageBtn").addEventListener("click", async () => {
    ...
    if (!text) { document.getElementById("messageError").style.display = "block"; return; }
    document.getElementById("messageError").style.display = "none";

    const username = sessionStorage.getItem("loggedInUser");
    const name = sessionStorage.getItem("loggedInName");
    if (!username) { alert("You must be logged in to send a message!"); return; }

    const email = await getUserEmail(username);
    const otherEmail = await getOtherUserEmail(username);
    const timestamp = new Date().toISOString();

    await addDoc(collection(db, "messages"), { authorUsername: username, authorName: name, text, timestamp, email });
...
export async function displayLatestMessages() {
  ...
  messagesSnap.forEach(docSnap => {
    ...
    if ((now - new Date(msg.timestamp)) / (1000 * 60 * 60) <= 24) recentMessages.push(msg);
  });
